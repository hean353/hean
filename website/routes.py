...
def generate_data():
    ...
    if refrence is None:
        flash("Reference not found in session.", "error") 
        return redirect(url_for('routes.base'))
    ...
    if not indexNumber:
        flash("Index number not found in session.", "error")
        return redirect(url_for('routes.base'))
...
def result():
    ...
    if not data:
        flash("Sorry, it seems there is an issue with the data you uploaded. Check your index number and try again. You can contact 0791196121.", category="error")
        return redirect(url_for("routes.base"))
